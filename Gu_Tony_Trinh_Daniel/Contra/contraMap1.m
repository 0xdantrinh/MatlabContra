%contraMap1 made by Tongda Gu and Daniel Trinh
%main function of Contra, this is the first stage of Contra I
%12/18/2014

function contraMap1(lives,bombs,score)
...
%it is tall and Key Press Functions
IB=imread('Cont1.png');
background=axes('position',[0 0 24.29 1]);
imagesc(IB);
axis off
...
while d==0
    ...
    %Initialize the different Spawn Cases  
    if bpos>-8.400
        spawnpos1=1
    end
    if bpos<=-8.4000&&bpos>=-11.80
        spawnpos1=0
        spawnpos2=1
    end
    if bpos<-11.80&&bpos>=-12.980
        spawnpos2=0
    end
    if bpos<-12.980&&bpos>=13.3000
        spawnpos3=1
    end
    if bpos<-13.300
        spawnpos3=0
    ...
    %Set the End Level Condition
    if bpos<=-22.8200
        close all
        clearvars -except lives bombs hiscore
        contraMap3(lives,bombs,hiscore)
    end
    ...
    if jump==1
       ...
       %player position and the platforms
        for jj=1:21
        if abs(ypos-tplatposy(jj))<.02+2*eps & xpos<(ppos(jj)+tplatleng(jj)) & xpos>ppos(jj)
            platform=jj;
            platformstate=1;
            gravity=0;
        end
    ...
    function spawnGunner
          if spawnpos1==1||spawnpos2==1||spawnpos3==1
...
%         %Randomly Spawn this guy with an odds of 1/4
    ...
    function loseLife
        ...
        image=I;
        alpha=1
        set(contraman,'position',[xpos ypos .15 .12])
    end
